...
export default function CsvImportPage() {
  ...
  const downloadTemplate = () => {
    // Header는 한국어/영어 혼용 그대로 유지(데이터 파서 호환)
    const headers = [
  ...
  const handleImport = async () => {
    if (!parsedProducts.length) {
      alert(t("csv.noValidRows", "No valid rows."));
      return;
    }
    ...
    try {
      ...
      alert(
        t("csv.doneSummary",
          ...
          {
            total,
            mode: overwriteMode ? "Overwrite document" : "Merge update",
            tags: replaceTags ? "Use CSV / replace" : "Ignore CSV (keep existing)",
            cats: replaceCategories ? "Use CSV / replace" : "Ignore CSV (keep existing)"
          }
        )
      );
    } catch (e) {
  ...
  return (
    <div style={{ maxWidth: 980, margin: "0 auto", padding: 16 }}>
      <h1 style={{ margin: 0, marginBottom: 12, fontSize: 22, fontWeight: 700 }}>
        {t("csv.title", "CSV Upsert (Insert/Update)")}
      </h1>

      {/* 업로드/옵션 */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 10, marginBottom: 12 }}>
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          <input type="file" accept=".csv,text/csv" onChange={onFile} aria-label={t("csv.pickFileAria","Pick CSV file")} />
          <button
          ...
          <span style={{ fontSize: 12, color: "#6b7280" }}>
            {fileName ? t("csv.selectedFile","Selected: {{name}}", { name: fileName }) : t("csv.hint","CSV (UTF-8, BOM recommended). TSV is OK.")}
          </span>
        </div>
        ...
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
          ...
          >
            {progress.running
              ? t("csv.processing","Processing… ({{done}}/{{total}})", { done: progress.done, total: progress.total })
              : t("csv.runUpsert","Run upsert ({{n}})", { n: parsedProducts.length })}
          </button>
        </div>
      </div>
...
1038756\t전통문양 봉투 2매입\t1,000\t전통 | 봉투\t문구/팬시\t포장용품`
      ...
      <h3 style={{ marginTop: 10, marginBottom: 8 }}>
        {t("csv.preview","Preview ({{n}} rows)", { n: parsedProducts.length })}
      </h3>
      {!parsedProducts.length ? (
        <div style={{ color: "#6b7280" }}>{t("csv.noParsed","No parsed rows.")}</div>
      ) : (
        <div style={{ overflowX: "auto", border: "1px solid #e5e7eb", borderRadius: 8 }}>
          ...
          {parsedProducts.length > 200 && (
            <div style={{ padding: 8, fontSize: 12, color: "#6b7280" }}>
              {t("csv.previewMore","Preview shows top 200 rows. All {{n}} rows will be processed.", { n: parsedProducts.length })}
            </div>
          )}
        </div>
      )}
