...
import {
  ...
  writeBatch, deleteDoc, serverTimestamp, arrayUnion, arrayRemove
...
const buildCsv = (rows) => {
  const header = [
    "상품ID","상품명","상품코드","가격","태그","대분류(categoryL1)","중분류(categoryL2)","이미지URL","링크"
  ].map(csvEscape).join(",");
  const lines = [header];
  rows.forEach((p) => {
    lines.push(
...
const downloadText = (content, filename, mime = "text/csv;charset=utf-8") => {
  const blob = new Blob(["\uFEFF" + content], { type: mime }); // BOM 포함 (엑셀 호환)
...
export default function ProductsAdminPage() {
  ...
  const handleBulkAddTags = async () => {
    ...
    try {
      ...
      setTagInput("");
      alert(`태그 추가 완료: ${selected.size}개`);
    } catch (e) {
      console.error(e);
      alert(e?.message || "태그 추가 실패");
    }
  };
  ...
  const handleBulkRemoveTags = async () => {
    ...
    if (selected.size === 0) return alert("선택된 항목이 없습니다.");
    if (!window.confirm(`선택 ${selected.size}개에서 태그 [${tokens.join(", ")}] 를 삭제할까요?`)) return;
    ...
    try {
      ...
      setTagInput("");
      alert(`태그 삭제 완료: ${selected.size}개`);
    } catch (e) {
      console.error(e);
  ...
  const handleBulkSetCategory = async () => {
    if (!catL1 || !catL2) return alert("대분류/중분류를 먼저 선택하세요.");
    if (selected.size === 0) return alert("선택된 항목이 없습니다.");
    if (!window.confirm(`선택 ${selected.size}개에 카테고리\n${catL1} > ${catL2}\n지정할까요?`)) return;
    ...
    try {
      ...
      setItems((prev) =>
        prev.map((p) =>
          selected.has(p.id) ? { ...p, categoryL1: catL1, categoryL2: catL2 } : p
        )
      );
      alert(`카테고리 지정 완료: ${selected.size}개`);
    } catch (e) {
      console.error(e);
      alert(e?.message || "카테고리 지정 실패");
    }
  };
  ...
  const downloadCsv = (onlySelected = false) => {
    ...
    downloadText(csv, onlySelected ? `선택상품_${today}.csv` : `필터상품_${today}.csv`);
  };
              ...
              <div key={p.id} style={{ border: "1px solid #e5e7eb", borderRadius: 10, padding: 10, display: "grid", gridTemplateColumns: "28px 80px 1fr", gap: 12, alignItems: "center" }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 12, color: "#6b7280" }}>
                    {p.categoryL1 || "-"} {p.categoryL2 ? `/ ${p.categoryL2}` : ""}
                  </div>
